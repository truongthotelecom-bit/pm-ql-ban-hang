...
export default function Transactions() {
  ...
  const handleCreateCustInline = async () => {
    if (!newCustPayload.ho_va_ten || !newCustPayload.so_dien_thoai) {
      message.error('Vui lòng điền đủ họ tên và số điện thoại khách!');
      return;
    }
  ...
  const handleSaveEditCust = async () => {
    if (!editCustPayload.ho_va_ten || !editCustPayload.so_dien_thoai) {
      message.error('Họ tên và Số điện thoại không được để trống!');
      return;
    }
    ...
    <div className="flex flex-col xl:flex-row gap-5 min-h-[650px] animate-fadeIn">
      ...
      <div className="w-full xl:w-[40%] p-4 rounded-2xl bg-[#0d1426]/70 border border-white/5 flex flex-col gap-4 shadow-xl backdrop-blur-md justify-between">
        <div className="space-y-4">
          <div className="flex justify-between items-center border-b border-white/5 pb-3">
            <span className="font-extrabold text-white text-xs tracking-wider uppercase">Chi tiết hồ sơ vụ việc</span>
            {activeFile && (
              <Button 
                type="primary" 
                size="small"
